#!/usr/bin/env python
"""Train a CNN for CIFAR100."""

__author__ = 'Zhang Shurui'
...
train_dataset = torchvision.datasets.CIFAR100(root=args.dataset_root, train=True, download=True,
            transform=Compose([
...
test_dataset = torchvision.datasets.CIFAR100(root=args.dataset_root, train=False, download=True, transform=to_tensor_and_normalize)
...
model = models.create_model(model_name=args.model, num_classes=100, in_channels=3)
...
writer = SummaryWriter(comment=('_cifar100_' + full_name))
...
def train(epoch):
    ...
    for batch in pbar:
        ...
        if args.mixup:
            inputs, targets = mixup(inputs, targets, num_classes=100)
